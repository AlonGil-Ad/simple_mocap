...
# Unpack Mocap Data Functions
    ...
    def __unpack_server_info(self, data, packet_size, major, minor):
        ...
        if (self.__nat_net_requested_version[0] == 0) and\
           (self.__nat_net_requested_version[1] == 0):
            # Don't print versions if print_level == 0 (Alon Gil-Ad)
    ...
    def __process_message(self, data: bytes, print_level=0):
        ...
        if message_id == self.NAT_FRAMEOFDATA:
            ...
            offset += offset_tmp
            # Don't print frame number if print_level == 0 (Alon Gil-Ad)
            ...
            offset += offset_tmp
            # Don't print Data Descriptions if print_level == 0 (Alon Gil-Ad)
            if print_level > 0:
                print("Data Descriptions:\n")
                # get a string version of the data for output
                data_descs_str = data_descs.get_as_string()
                print(" %s\n" % (data_descs_str))
            # Send data descriptions to listener (Alon Gil-Ad)
    ...
    def send_request(self, in_socket, command, command_str, address):
        ...
        if command == self.NAT_REQUEST_MODELDEF or command == self.NAT_REQUEST_FRAMEOFDATA: #type: ignore  # noqa E501
            ...
            tmp_version = [4, 2, 0, 0]
            # Don't print if print_level == 0 (Alon Gil-Ad)
            if self.print_level > 0:
    ...
    # Send data description request. Received via self.data_description_listener (Alon Gil-Ad)
    def request_data_descriptions(self):
        self.send_request(self.command_socket, self.NAT_REQUEST_MODELDEF, "",
